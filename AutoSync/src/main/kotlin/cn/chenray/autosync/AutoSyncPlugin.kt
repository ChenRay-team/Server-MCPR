...
class AutoSyncPlugin : JavaPlugin() {
    ...
    private fun init() {
        ...
        // 停止旧的定时任务，避免重载后重复执行
        if (syncTaskId != -1) {
            server.scheduler.cancelTask(syncTaskId)
        }

        syncManager = SyncManager(this)  // 重建（含新的 client）
    ...
    override fun onDisable() {
        ...
        if (fullyEnabled && ::syncManager.isInitialized && syncConfig.syncOnDisable) {
            try {
                syncManager.syncNow()
            } catch (e: Exception) {
